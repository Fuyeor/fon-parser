...
export function lex(
  ...
  const push = (
    ...
    if (tokens.length >= options.maxTokens) {
      if (!limitReported) {
        ...
        addDiagnostic(
          'E0001',
          `token limit exceeded (maximum ${options.maxTokens})`,
          start,
          end,
        );
      }
  ...
  while (index < source.length) {
    ...
    if (character === '/' && source[index + 1] === '*') {
      ...
      if (!closed)
        addDiagnostic('E0003', 'unterminated block comment', start, index);
      continue;
    }
    ...
    if (character === '`') {
      ...
      if (index - start > options.maxTokenLength) {
        addDiagnostic(
          'E0002',
          `token length exceeded (maximum ${options.maxTokenLength})`,
          start,
          index,
        );
      }
      ...
      if (source[index - 1] !== '`')
        addDiagnostic('E0004', 'unterminated backtick string', start, index);
      continue;
    }
  ...
  if (source.length > options.maxSourceLength) {
    addDiagnostic(
      'E0005',
      `source length exceeded (maximum ${options.maxSourceLength})`,
