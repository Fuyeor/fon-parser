...
describe("FON parser", () => {
  ...
  it("requires an explicit object for root annotations", () => {
    ...
    expect(result.diagnostics.some((diagnostic) => diagnostic.code === "E0101")).toBe(true);
  });

  it("accepts mixed and trailing separators", () => {
    for (const source of ["{ a = 1, b = 2 }", "{ a = 1 , b = 2 }", "{ a = 1\nb = 2 }", "{ a = 1,\n b = 2 }", "{ a = 1,\nb = 2, }"]) {
      const result = parse(source);
      expect(result.hasErrors(), source).toBe(false);
      expect(result.document.root.kind).toBe("explicit-object");
      if (result.document.root.kind === "explicit-object") expect(result.document.root.members).toHaveLength(2);
    }
  });
  ...
  it("ignores line and block comments outside backtick strings", () => {
    ...
    if (result.document.root.kind !== "implicit-object") throw new Error("expected implicit object");
  ...
  it("preserves duplicate members and source-backed spans", () => {
    ...
    const result = parse(source);
    expect(result.hasErrors()).toBe(false);
    if (result.document.root.kind !== "explicit-object") throw new Error("expected explicit object");
  ...
  it("classifies values without coercing unknown atoms", () => {
    const result = parse("package = @fer/std\nversion = 0.1.0\npath = ./docs/index.md\ncolor = #AEA4E4\nlicense = mit\n");
    expect(result.hasErrors()).toBe(false);
    if (result.document.root.kind !== "implicit-object") throw new Error("expected implicit object");
    ...
    expect(values.map((value) => value?.kind)).toEqual(["unknown", "unknown", "unknown", "unknown", "unknown"]);
    expect(values.map((value) => value?.kind === "unknown" ? value.shape : "")).toEqual([
      "package-like", "version-like", "path-like", "color-like", "bare-atom",
    ]);
  });

  it("retains string interpolation and regex spans", () => {
    const result = parse("message = `Hello, {name}!`\nidentifier = /^[a-z0-9-]+$/i\n");
    expect(result.hasErrors()).toBe(false);
    if (result.document.root.kind !== "implicit-object") throw new Error("expected implicit object");
    const first = result.document.ast.members[result.document.root.members[0] ?? -1];
    const second = result.document.ast.members[result.document.root.members[1] ?? -1];
    if (first?.kind !== "binding" || second?.kind !== "binding") throw new Error("expected bindings");
    ...
    if (string?.kind === "string") {
      expect(string.parts.map((part) => part.kind)).toEqual(["text", "interpolation", "text"]);
      expect(text(result.document, string.parts[1]?.expression ?? { start: 0, end: 0 })).toBe("name");
    }
  ...
  it("parses nested values, schemas, types, enums, and annotations", () => {
    ...
    const mode = result.document.ast.members[result.document.root.members[2] ?? -1];
    const other = result.document.ast.members[result.document.root.members[3] ?? -1];
    if (mode?.kind !== "binding" || other?.kind !== "binding") throw new Error("expected enum bindings");
  ...
  it("reports malformed input and enforces resource limits", () => {
    ...
    expect(malformed.diagnostics.some((diagnostic) => diagnostic.code === "E0103" || diagnostic.code === "E0205")).toBe(true);

    const limited = parse("a = [1, 2, 3]", { maxCollectionItems: 2 });
    expect(limited.hasErrors()).toBe(true);
    expect(limited.diagnostics.some((diagnostic) => diagnostic.code === "E0006")).toBe(true);
  });

  it("parses UTF-8 bytes without a runtime dependency", () => {
    const bytes = new Uint8Array([110, 97, 109, 101, 32, 61, 32, 96, 0xe7, 0x8e, 0xa5, 96]);
  ...
  it("visits indexed nodes in source order", () => {
    ...
    visit(result.document, {
      visitMember: (member) => { if (member.kind === "binding") seen.push(text(result.document, member.key.raw)); },
    });
