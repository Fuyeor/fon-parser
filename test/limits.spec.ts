...
describe("FON parser limits and diagnostics", () => {
  it("bounds nesting depth", () => {
    ...
    expect(result.diagnostics.some((diagnostic) => diagnostic.code === "E0008")).toBe(true);
  });

  it("bounds lexical token count", () => {
    const result = parse("a = 1\nb = 2", { maxTokens: 4 });
    expect(result.hasErrors()).toBe(true);
    expect(result.diagnostics.some((diagnostic) => diagnostic.code === "E0001")).toBe(true);
  });

  it("reports unterminated strings and block comments", () => {
    const stringResult = parse("value = `unterminated");
    expect(stringResult.diagnostics.some((diagnostic) => diagnostic.code === "E0004")).toBe(true);
    const commentResult = parse("/* unterminated");
    expect(commentResult.diagnostics.some((diagnostic) => diagnostic.code === "E0003")).toBe(true);
  });
  ...
  it("fails fast for invalid options", () => {
    ...
    expect(() => parse("", { maxTokens: Number.POSITIVE_INFINITY })).toThrow(RangeError);
  });
});
